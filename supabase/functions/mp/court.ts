...
import { db, ok, err, json, authedUserId, ownerFilter, safeClientId, computeStreak, loadRosterPool, rosterRevealFor } from "./shared.ts";
...
async function courtReveal(courtDay: CourtDay, attempt: any) {
  ...
  return Array.isArray(snapshot) ? rosterRevealFor(snapshot as PoolEntry[]) : null;
}
